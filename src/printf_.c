...
void putc_VirCom (char);         // blocking put char; used by printf_()
...
void putc_VirCom (char c)
{
	CDC_Send_DATA((unsigned char*) &c,1);DelayMs(1);
}
...
int printf_(const char *format, ...)
{
	...
	va_start(arg, format);
	vfprintf_((&putc_VirCom), format, arg);
	va_end(arg);
